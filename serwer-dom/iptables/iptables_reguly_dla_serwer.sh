...
# FTP
# FTP tryb aktywny
$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.4 -p tcp --dport 20:21 -j ACCEPT
$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.4 -p udp --dport 20:21 -j ACCEPT
# $f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.4 -p tcp --dport 21 -j ACCEPT
# $f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.4 -p tcp --dport 21 -j ACCEPT

# FTP tryb pasywny
$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.4 -p tcp --dport 1024:1025 -j ACCEPT
$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.4 -p udp --dport 1024:1025 -j ACCEPT
#$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.4 -p tcp --dport 1025 -j ACCEPT
#$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.4 -p udp --dport 1025 -j ACCEPT

echo "Reguła SSH"
# SSH
$f -A INPUT -s 0/0 -p tcp --dport 10025 -j ACCEPT
$f -A INPUT -s 0/0 -p udp --dport 10025 -j ACCEPT
...
echo "Reguła Samba"
# Samba
$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.9 -p udp --dport 137 -j ACCEPT
$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.9 -p udp --dport 138 -j ACCEPT
$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.9 -p tcp --dport 139 -j ACCEPT
$f -A INPUT -m iprange --src-range 192.168.1.2-192.168.1.9 -p tcp --dport 445 -j ACCEPT

echo "Reguła APACHE"
# APACHE
$f -A INPUT -s 0/0 -p tcp --dport 80 -j ACCEPT
$f -A INPUT -s 0/0 -p udp --dport 80 -j ACCEPT
$f -A INPUT -s 0/0 -p tcp --dport 443 -j ACCEPT
$f -A INPUT -s 0/0 -p tcp --dport 443 -j ACCEPT

echo "Reguła RTORRENT"
# RTORRENT
$f -A INPUT -s 0/0 -p tcp --dport 53211 -j ACCEPT
$f -A INPUT -s 0/0 -p udp --dport 53211 -j ACCEPT

echo "Reguła DLNA"
# DLNA START
# odblokowanie broadcast-u uPnP
$f -A OUTPUT -o eth0 -d 239.255.255.250 -j ACCEPT
$f -A OUTPUT -o eth0 -s 239.255.255.250 -j ACCEPT
$f -A INPUT -i eth0 -d 239.255.255.250 -j ACCEPT
$f -A INPUT -i eth0 -s 239.255.255.250 -j ACCEPT
 
# odblokowanie serwera miniDLNA
$f -A OUTPUT -o eth0 -p tcp --sport 8200 -j ACCEPT
$f -A INPUT -i eth0 -p tcp --dport 8200 -j ACCEPT

# odblokowanie uPnP
$f -A INPUT -i eth0 -p udp --dport 1900 -j ACCEPT
$f -A OUTPUT -o eth0 -p udp --sport 1900 -j ACCEPT
## KONIEC STOP
